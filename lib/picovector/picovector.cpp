...
namespace picovector {
  ...
  void build_nodes(path_t *path, rect_t *tb, mat3_t *transform, unsigned aa) {
    vec2_t offset = tb->tl();
  ...
  rect_t render_nodes(rect_t *tb, unsigned aa) {
    int minx = tb->w;
    int miny = tb->h;
    int maxx = 0;
    int maxy = 0;
  ...
  void render(shape_t *shape, image_t *target, mat3_t *transform, brush_t *brush) {
    ...
    unsigned aa = (unsigned)target->antialias();
  ...
  void build_glyph_nodes(glyph_path_t *path, rect_t *tb, mat3_t *transform, unsigned aa) {
    vec2_t offset = tb->tl();
    // start with the last point to close the loop, transform it, scale for antialiasing, and offset to tile origin
